...
**All previously Partial items have been wired:** judge node (stable prompt format, prompt hash versioning, structured parsing), refine node (deterministic fidelity flag with correct priority order), metadata node (schema validation, tier-2 fallback logging), hierarchical chunking (wired as `strategy: hierarchical` in pipeline runner), Docling ingest (DoclingIngestError no longer swallowed in PDF fallback path), Looking Glass ledger (richer summary with `docs_unique`/`runs_last_24h`, tenant/project filters on in-flight and failures), pipeline HITL resume (double-resume guard, `resumed_at` provenance recorded).
