...
def deterministic_chunk_id(*, doc_id: str, doc_version: str, content_hash: str, chunk_index: int) -> str:
    # Qdrant point IDs must be an integer or a UUID.
    # We derive a deterministic UUID from a sha256 digest of stable inputs.
    raw = f"{doc_id}:{doc_version}:{chunk_index}:{content_hash}".encode("utf-8")
