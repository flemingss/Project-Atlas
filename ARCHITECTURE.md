...
- **`cleanup.py`** - Deterministic markdown cleanup node
  - Five built-in transforms plus nine configurable builtin extraction-artifact fixes (`html_unescape`, `fix_ligatures`, `strip_zero_width_chars`, `strip_bullet_glyphs`, `strip_page_numbers`, `normalize_superscripts`, `dedupe_table_spans`, `strip_repetitive_lines`, `strip_repeated_headings`) — eight are ON by default (`strip_repeated_headings` in its safe `scope: title` mode); only `strip_repetitive_lines` defaults to OFF because it removes content
...
Current automated coverage (**787 tests across 58 files** — 781 in the CI unit shard, 6 `integration`-marked tests in a separate job that skip without cached models; 2026-08-30):
...
Run tests:
```bash
pytest -q                    # All tests (787; CI shards them, see .github/workflows/ci.yml)
