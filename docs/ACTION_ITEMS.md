...
- **Candidate image built and measured 2026-08-30** (`project-atlas-atlas:docling-2.123`, scratch worktree `eval/docling-2.123`, locks regenerated cleanly; details on #65). On the real datasheet 2.123.1 is **better**: both phone numbers correct on both pages, the S650 panel misfire gone, the fi-ligature loss gone, same heading/table counts. On the synthetic fixture it is **worse**: `Section 1`/`Section 2` demoted from headings and interleaved mid-sentence, bullets merged onto one line. And in artifacts-path (offline) mode it **lost the smoke fixture's table** entirely, which the same version found in HF-cache mode — likely a different default layout model (`docling-layout-heron-onnx` appears in its artifacts). **Recommendation: hold 2.76.0 for v0.9.0; revisit 2.123.x with the layout model pinned to the PyTorch heron and re-run the smoke offline.** Operator GUI comparison still welcome: `docker tag project-atlas-atlas:docling-2.123 project-atlas-atlas:latest && docker compose up -d --no-build atlas` (and `docker compose build atlas` to go back).
...
- **Action:** keep generated fixtures (do not commit production manuals).
- ✅ **DONE 2026-08-30 (image gate):** `.github/workflows/image.yml` builds the image on every push to `main`, runs `scripts/image_smoke.py` inside it as the runtime user with `--network none` (manufactured PDF: doubled digits, two columns, spanned table header, glyph bullets, raised exponent, running headers), then runs the `integration` tests inside the image. Its first dry run found the image could not parse offline (fixed the same day). Extend the fixture whenever a real document shows a new extraction situation.
